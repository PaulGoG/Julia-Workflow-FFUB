...
function Calcul_citire_tastatura()
    println(" ")
    println("======================================================================================================================")
    println("Programul va calcula energia de separare a unei particule dintr-un nucleu, pentru amândouă urmând a fi introduse de la tastatură A-ul și Z-ul.")
    print("Apăsați tasta ENTER pentru a continua...")
    readline();
    println("Se încarcă librăria de date nucleare, vă rugăm așteptați introducerea datelor de la tastatură...")
    println(" ")
    df = CSV.File("AUDI95.csv"; delim=' ', ignorerepeated=true, header=["Z", "A", "Sym", "D", "σ"]) |> DataFrame
    global contor = true
    # Procesul de citire a datelor de la tastatura se va efectua pana cand se introduce un set de date corecte sau se opreste rularea programului
    while(contor)
